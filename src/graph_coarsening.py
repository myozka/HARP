...
def skipgram_coarsening_hs(recursive_graphs, recursive_merged_nodes, **kwargs):
    ...
    for level in range(levels - 1, -1, -1):
        ...
        if level == levels - 1:
            ...
            # copy vocab / wv.index2word from the coarser graph
            ...
            model.syn1 = np.zeros((len(model.wv.vocab), model.trainables.layer1_size), dtype=np.float32)
            for i in range(len(models[-1].syn1)):
                model.syn1[i] = models[-1].syn1[i]
            model.wv.syn0_lockf = np.ones(len(model.wv.vocab), dtype=np.float32)
...
def skipgram_coarsening_neg(recursive_graphs, recursive_merged_nodes, **kwargs):
    ...
    for level in range(levels - 1, -1, -1):
        ...
        if level == levels - 1:
            ...
            model.trainables.reset_weights(model.hs, model.negative, model.wv)
